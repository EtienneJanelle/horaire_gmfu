...
function personSection(personKey, person) {
  return (
    <div class="personsection" data-person={personKey}>
      <div class="info">
        <div className="state">
          {/* Whether changes were made to the grid since last period or since generating schedule */}
          {(()=>{
      ...
      <div class="grids">
        ...
        {(()=>{
          ...
          if (horaireState == "inputs") {
            if (person.role == "med") {
              // asked ADM
              output.push(Array(4).fill(4).map((_, weekKey) => (
            ...
            if (person.role == "med" || person.role == "ips") {
              // asked Periods
              output.push(Array(4).fill(4).map((_, weekKey) => (
            ...
            return dispogrid({
              ...
              callback: (position, value, cellEl) => {
                ...
                cellEl.closest(".personsection").querySelector(".state").innerHTML = 
                  '<div><ion-icon name="checkmark-circle"></ion-icon>Calculé et modifié</div>'
                updateOutputCounts(personKey, cellEl.closest(".personsection"))
              }
            })
          } else return <></>
        })()}
...
function renderDispoPage() {
  if (horaireState == "inputs") {
      ...
      <div className="peoplepage">
  ...
  
        {
          ...
          Object.entries(data.people).map(([personKey, person], i) => {
            ...
            const personSectionEl = personSection(personKey, person)
    ...
    return (
      <div className="peoplepage outputs">
        <div className="peopleinfo">
          <div className="content">
          ...
          <div className="actions">
            <div className="button" onclick={nextWeek}>Prochaine période</div>
          </div>
        </div>
  
        {
          ...
          Object.entries(data.people).map(([personKey, person], i) => {
            if (person.role != activePage) return <></>
            const personSectionEl = personSection(personKey, person)
            updateOutputCounts(personKey, personSectionEl)
            return personSectionEl
          })
        }
      </div>
