...
function calcSchedule() {
  ...
  data.global.schedule.availableSupervisors = {}
  data.global.schedule.openSupervisors = {}
  data.global.schedule.totalAvailableSupervisors = {}
  ...
  for (let personKey in data.people) {
    ...
    person.schedule.supervising = Array(84).fill([])
  }
  ...
  for (let weekKey = 0; weekKey < 4; weekKey++) {
    ...
    for (let execLimit = 100; execLimit >= 0; execLimit--) {
      if ((()=>{
        // count available supervisors per activty for each period and a totalfor the week
        for (let activityKey in data.activities) {
          if (data.activities[activityKey].supervision != true) continue
          data.global.schedule.availableSupervisors[activityKey] = Array(84).fill(0)
          data.global.schedule.openSupervisors[activityKey] = Array(84).fill(0)
          data.global.schedule.totalAvailableSupervisors[activityKey] = 0
        }
        data.global.schedule.availableSupervisors["ipsBUR"] = Array(84).fill(0)
        data.global.schedule.openSupervisors["ipsBUR"] = Array(84).fill(0)
        data.global.schedule.totalAvailableSupervisors["ipsBUR"] = 0
        for (let personKey in data.people) {
          const person = data.people[personKey]
          ...
          if (person.role == "med") {
            for (let periodKey of periodsOfWeek(weekKey)) {
              if (person.schedule.burOnly[periodKey]) continue
              let activityKey = person.schedule.dispo[periodKey]
              if (activityKey == "dispo") activityKey = "BUR"
              if (data.activities[activityKey].supervision != true) continue
              if (person.competences[activityKey] != true) continue
              data.global.schedule.availableSupervisors[activityKey][periodKey] += data.activities[activityKey].supervisionLimit - person.schedule.supervising[periodKey].length
              data.global.schedule.totalAvailableSupervisors[activityKey] += data.activities[activityKey].supervisionLimit - person.schedule.supervising[periodKey].length
              if (person.schedule.supervising[periodKey].length > 0) data.global.schedule.openSupervisors[activityKey][periodKey] += data.activities[activityKey].supervisionLimit - person.schedule.supervising[periodKey].length
            }
          } else if (person.role == "ips") {
            for (let periodKey of periodsOfWeek(weekKey)) {
              if (person.schedule.burOnly[periodKey]) continue

              let activityKey = person.schedule.dispo[periodKey]
              if (activityKey != "BUR" && activityKey != "dispo") continue
              if (person.competences["BUR"] != true) continue
              data.global.schedule.availableSupervisors["ipsBUR"][periodKey] += data.activities["BUR"].supervisionLimit - person.schedule.supervising[periodKey].length
              data.global.schedule.totalAvailableSupervisors["ipsBUR"] += data.activities["BUR"].supervisionLimit - person.schedule.supervising[periodKey].length
              if (person.schedule.supervising[periodKey].length > 0) data.global.schedule.openSupervisors["ipsBUR"][periodKey] += data.activities["BUR"].supervisionLimit - person.schedule.supervising[periodKey].length
            }
          }
        }

        // count how many available spots are left during the day in each student's schedule: if there is only 3, assign them as "bur"
        for (let personKey in data.people) {
          const person = data.people[personKey]
          if (person.role != "res") continue
          let totalDispo = 0
          for (let periodKey of periodsOfWeek(weekKey)) {
            if (periodKey % 3 == 2) continue
            if (person.schedule.dispo[periodKey] != "dispo") continue
            if (data.global.schedule.availableSupervisors["BUR"][periodKey] == 0) continue
            totalDispo++
          }
          if (totalDispo <= 3) {
            for (let periodKey of periodsOfWeek(weekKey)) {
        ...
        // incrementing comp numbers
        for (let personKey in data.people) {
          const person = data.people[personKey]
          if (person.role != "res") continue
          for (let activityKey in data.activities) {
            const activity = data.activities[activityKey]
            if (activity.supervision != true) continue
            if (person.schedule.compSup[activityKey] == undefined) person.schedule.compSup[activityKey] = 0
            person.schedule.compSup[activityKey] += activity.supCompIncrease
          }
        }

        // starting with activities with the lowest supervisors available, assign students (role:res) whose compensetion (schedule.comp<activity>) number for that activity is the lowest
        const activityKeys = Object.keys(data.activities).filter(activityKey => data.activities[activityKey].supervision)
        activityKeys.sort((keyA, keyB) => {data.global.schedule.totalAvailableSupervisors[keyB] - data.global.schedule.totalAvailableSupervisors[keyA]})
        for (let activityKey of activityKeys) {
          if (data.global.schedule.totalAvailableSupervisors[activityKey] == 0) continue
          let highestCompValue = -1
          let highestCompPerson
          for (let personKey in data.people) {
            const person = data.people[personKey]
            if (person.role != "res") continue
            for (let periodKey of periodsOfWeek(weekKey)) {
              if (periodKey % 3 == 2) continue
              if (person.schedule.dispo[periodKey] != "dispo") continue
              if (person.schedule.compSup[activityKey] > highestCompValue) {
                highestCompPerson = person
                highestCompValue = person.schedule.compSup[activityKey]
              }
            }
          }
          if (highestCompPerson == undefined) continue
          for (let periodKey in randomiseArray(periodsOfWeek(weekKey))) {
            if (periodKey % 3 == 2) continue
            if (highestCompPerson.schedule.dispo[periodKey] != "dispo") continue
            assignPeriod(data.people[highestCompPerson], periodKey, activityKey)
            return true
          }
        }
  ...
  function assignPeriod(person, periodKey, activityKey) {
    ...
    if (person.role == "res") {
      // find supervising doctor
      let chosenSupervisor
      let chosenSupervisorValue
      ...
      for (let periodKey of periodsOfWeek(weekKey)) {
        ...
        if (activityKey == "dispo") activityKey = "BUR"
        if (person.competences[activityKey] != true) continue
        if (person.schedule.supervising[periodKey].length >= data.activities[activityKey].supervisionLimit) continue
        if (person.schedule.supervising[periodKey].length > 0) {
          chosenSupervisor = person;
          continue
        }
        if (person.schedule.compSupMed > chosenSupervisorValue) {
          chosenSupervisor = person
        }
        if (chosenSupervisor.schedule.dispo[periodKey] == "dispo") assignPeriod(chosenSupervisor, periodKey, activityKey);
        chosenSupervisor.schedule.supervising[periodKey].push(person)
        if (chosenSupervisor.schedule.supervising[periodKey].length == data.activities[activityKey].supervisionLimit && ["bureau", "srv", "clinique"].includes(activity.type)) data.global.schedule.availableOffices[periodKey]++
