...
function updateOutputCounts(personKey, element) {
...
function updateDispoCounts(personKey, element) {
  ...
  for (let weekKey of [0,1,2,3]) {
    let activityCounts = {}
    for (let activityKey in data.activities) {activityCounts[activityKey] = 0}
    ...
    if (person.role == "med") {
      if (alreadyAssignedOffices > 0) {
        element.querySelector('.assignedcount.assignedBUR[data-week="' + weekKey + '"]').appendChild(<div>-{activityCounts["BUR"] + activityCounts["SRV"]} déjà assignés</div>)
      }
      if (alreadyAssignedADM > 0) {
        element.querySelector('.assignedcount.assignedADM[data-week="' + weekKey + '"]').appendChild(<div>-{activityCounts["ADM"]} déjà assignés</div>)
      }
      if (parseInt(person.inputs.askedPeriods[weekKey]) + parseInt(person.inputs.askedADM[weekKey]) - activityCounts["ADM"] - activityCounts["BUR"] - activityCounts["SRV"] > activityCounts["dispo"]) {
        console.log(parseInt(person.inputs.askedPeriods[weekKey]) + parseInt(person.inputs.askedADM[weekKey]) - activityCounts["ADM"] - activityCounts["BUR"] - activityCounts["SRV"], activityCounts["dispo"])
      ...
      if (parseInt(person.inputs.askedPeriods[weekKey]) - alreadyAssignedOffices > dispoCount) {
...
function fillData() {
  ...
  if(data.global.inputs.supervising == undefined) {data.global.inputs.supervising = Array(84).fill([])}
  ...
  for (let personKey in data.people) {
    ...
    if(person.trackers.compSup == undefined) person.trackers.compSup = {}
    if(person.trackers.compSupMed == undefined) person.trackers.compSupMed = 0
